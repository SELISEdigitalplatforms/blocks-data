...
/// </summary>
[Collection("Mongo")]
public class ObjectManagementServiceTests : IDisposable
{
    private readonly IMongoDatabase _db;
    private readonly ObjectAccessRepository _accessRepository;
    private readonly ObjectManagementService _management;

    public ObjectManagementServiceTests(MongoFixture fixture)
    {
        ...
        _management = new ObjectManagementService(provider.Object, _accessRepository, new ObjectAccessResolver(_accessRepository));
    ...
    private static ObjectAccessPolicy Policy(
        ...
        ObjectPrincipalType principalType = ObjectPrincipalType.Role,
        string? principalId = "auditors",
        ObjectPermission permission = ObjectPermission.View,
        ObjectEffect effect = ObjectEffect.Allow) => new()
        {
    ...
    private async Task<List<ObjectAuditLog>> Audit(string resourceId) =>
    ...
    public async Task Granting_on_a_resource_that_does_not_exist_is_refused()
    {
        ...
        result.Status.Should().Be(ObjectAccessOperationStatus.ResourceNotFound);
    }
    ...
    public async Task A_caller_without_manage_cannot_grant_and_the_attempt_is_audited()
    {
        ...
        result.Status.Should().Be(ObjectAccessOperationStatus.NotPermitted);
    ...
    public async Task The_owner_of_a_resource_holds_manage_without_an_explicit_entry()
    {
        ...
        result.Status.Should().Be(ObjectAccessOperationStatus.Succeeded);
    }
    ...
    public async Task An_explicit_manage_grant_is_enough_to_administer_a_resource_owned_by_someone_else()
    {
        ...
        (await _management.GrantAccessAsync(Policy())).Status.Should().Be(ObjectAccessOperationStatus.Succeeded);
    }
    ...
    public async Task A_deny_aimed_at_the_owner_is_refused_rather_than_stored()
    {
        ...
        var result = await _management.GrantAccessAsync(
            Policy(principalType: ObjectPrincipalType.User, principalId: "user-1", effect: ObjectEffect.Deny));

        result.Status.Should().Be(ObjectAccessOperationStatus.SelfDenyRejected);
    ...
    public async Task A_deny_aimed_at_someone_who_is_not_the_owner_is_allowed()
    {
        ...
        var result = await _management.GrantAccessAsync(
            Policy(principalType: ObjectPrincipalType.User, principalId: "user-2", effect: ObjectEffect.Deny));
    ...
    public async Task A_grant_without_a_principal_is_refused_for_every_kind_that_needs_one(ObjectPrincipalType principalType)
    {
        await OwnedDirectory();

        var result = await _management.GrantAccessAsync(Policy(principalType: principalType, principalId: null));

        result.Status.Should().Be(ObjectAccessOperationStatus.PrincipalRequired);
    }
    ...
    public async Task An_everyone_grant_needs_no_principal()
    {
        ...
        var result = await _management.GrantAccessAsync(
            Policy(principalType: ObjectPrincipalType.Everyone, principalId: null));
    ...
    public async Task Updating_an_entry_replaces_it_and_keeps_its_creation_time()
    {
        ...
        var edit = Policy(permission: ObjectPermission.Edit);
        ...
        result.Status.Should().Be(ObjectAccessOperationStatus.Succeeded);
        var stored = (await _accessRepository.GetByResourceAsync("dir-1")).Single();
        stored.Permission.Should().Be(ObjectPermission.Edit);
    ...
    public async Task Updating_an_entry_that_does_not_exist_is_refused()
    {
        ...
        (await _management.UpdateAccessAsync(edit)).Status.Should().Be(ObjectAccessOperationStatus.PolicyNotFound);
    }
    ...
    public async Task Revoking_an_entry_that_does_not_exist_is_refused()
    {
        ...
        (await _management.RevokeAccessAsync("dir-1", Guid.NewGuid().ToString()))
            .Status.Should().Be(ObjectAccessOperationStatus.PolicyNotFound);
    }
    ...
    public async Task Sharing_creates_an_allow_entry_and_is_logged_as_a_share()
    {
        ...
        var result = await _management.ShareObjectAsync(
            "dir-1", ObjectResourceType.Directory, ObjectPrincipalType.User, "user-9", ObjectPermission.Download);

        result.Status.Should().Be(ObjectAccessOperationStatus.Succeeded);

        var stored = (await _accessRepository.GetByResourceAsync("dir-1")).Single();
        stored.Effect.Should().Be(ObjectEffect.Allow);
        stored.Permission.Should().Be(ObjectPermission.Download);
    ...
    public async Task Sharing_without_manage_is_refused()
    {
        ...
        var result = await _management.ShareObjectAsync(
            "dir-1", ObjectResourceType.Directory, ObjectPrincipalType.User, "user-9", ObjectPermission.View);
    ...
    public async Task Inheritance_cannot_be_switched_off_while_nothing_else_grants_access()
    {
        ...
        result.Status.Should().Be(ObjectAccessOperationStatus.WouldOrphanResource);
    ...
    {
        await OwnedDirectory();
        await _management.GrantAccessAsync(Policy(principalType: ObjectPrincipalType.User, principalId: "user-1"));
    ...
    public async Task A_deny_only_entry_does_not_count_as_granting_access()
    {
        ...
        (await _management.ToggleInheritanceAsync("dir-1", inherits: false))
            .Status.Should().Be(ObjectAccessOperationStatus.WouldOrphanResource);
    }
    ...
    public async Task Inheritance_can_always_be_switched_back_on()
    {
        ...
        (await _management.ToggleInheritanceAsync("dir-1", inherits: true))
            .Status.Should().Be(ObjectAccessOperationStatus.Succeeded);
    }
