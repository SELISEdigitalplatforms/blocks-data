...
/// </summary>
[Collection("Mongo")]
public class ObjectListingServiceTests : IDisposable
{
    private readonly IMongoDatabase _db;
    private readonly ObjectAccessRepository _accessRepository;
    private readonly ObjectListingService _listing;

    public ObjectListingServiceTests(MongoFixture fixture)
    {
        ...
        provider.Setup(p => p.GetCollection<ObjectAccessPolicy>(It.IsAny<string>()))
            .Returns((string n) => _db.GetCollection<ObjectAccessPolicy>(n));
        provider.Setup(p => p.GetCollection<ObjectAuditLog>(It.IsAny<string>()))
            .Returns((string n) => _db.GetCollection<ObjectAuditLog>(n));
    ...
    private async Task Grant(string resourceId, ObjectPermission permission, ObjectEffect effect = ObjectEffect.Allow,
        ObjectPrincipalType principalType = ObjectPrincipalType.User, string? principalId = "user-1")
    ...
    public async Task A_non_inheriting_child_restricted_to_another_user_is_hidden()
    {
        ...
        await Grant("file-1", ObjectPermission.View, principalId: "user-2");
    ...
    public async Task Listing_a_directory_the_caller_cannot_view_returns_nothing()
    {
        ...
        await Grant("private", ObjectPermission.View, principalId: "user-2");
    ...
    public async Task An_inheriting_child_with_its_own_deny_is_hidden_even_though_the_parent_is_visible()
    {
        ...
        await Grant("file-2", ObjectPermission.View, ObjectEffect.Deny);
    ...
    public async Task A_child_that_stops_inheriting_is_hidden_unless_it_grants_access_directly()
    {
        ...
        await Grant("file-1", ObjectPermission.View);
    ...
    {
        await AddFile("file-1", "doc.txt");
        await Grant("file-1", ObjectPermission.Download);
    ...
    public async Task A_page_is_filled_even_when_most_children_are_invisible()
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            ...
            if (i % 4 != 0) await Grant($"file-{i:D2}", ObjectPermission.View, ObjectEffect.Deny);
        }
