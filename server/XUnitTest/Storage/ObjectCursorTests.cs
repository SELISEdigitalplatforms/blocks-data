...
public class ObjectCursorTests
{
    [Fact]
    public void A_cursor_round_trips_through_its_encoding()
    {
        var cursor = new ObjectCursor { Type = StructureType.Directory, Name = "Reports", ItemId = "dir-1" };

        var decoded = ObjectCursor.Decode(cursor.Encode());
    ...
    {
        var cursor = new ObjectCursor { Type = StructureType.File, Name = name, ItemId = "file-1" };

        ObjectCursor.Decode(cursor.Encode())!.Name.Should().Be(name);
    }
    ...
    public void A_missing_or_malformed_cursor_decodes_to_null_rather_than_throwing(string? encoded)
    {
        ...
        ObjectCursor.Decode(encoded).Should().BeNull();
    }
    ...
    public void A_cursor_naming_an_unknown_type_decodes_to_null()
    {
        ...
        ObjectCursor.Decode(raw).Should().BeNull();
    }
    ...
    public void Directorys_sort_ahead_of_files()
    {
        ...
        ObjectCursor.Compare(StructureType.Directory, "zzz", "a", StructureType.File, "aaa", "b")
            .Should().BeNegative();
    }
    ...
    {
        ObjectCursor.Compare(StructureType.File, "alpha", "b", StructureType.File, "beta", "a")
            .Should().BeNegative();

        ObjectCursor.Compare(StructureType.File, "same", "a", StructureType.File, "same", "b")
            .Should().BeNegative();

        ObjectCursor.Compare(StructureType.File, "same", "a", StructureType.File, "same", "a")
            .Should().Be(0);
    }
    ...
    public void Name_ordering_is_ordinal_so_it_matches_the_database_sort()
    {
        ...
        ObjectCursor.Compare(StructureType.File, "Zebra", "a", StructureType.File, "apple", "b")
            .Should().BeNegative();
    }
    ...
    {
        var position = new ObjectCursor { Type = StructureType.File, Name = "m", ItemId = "id-5" };
