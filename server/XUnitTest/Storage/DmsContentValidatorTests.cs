...
/// </summary>
public class DmsContentValidatorTests
{
    ...
    public void A_grant_needs_a_principal_for_every_kind_except_everyone(ContentPrincipalType principalType)
    {
    ...
    {
        var validator = new ContentSearchRequestValidator();

        validator.Validate(new ContentSearchRequest { Query = "" }).IsValid.Should().BeFalse();
        validator.Validate(new ContentSearchRequest { Query = "report", Limit = 0 }).IsValid.Should().BeFalse();
        validator.Validate(new ContentSearchRequest { Query = "report", Limit = 20 }).IsValid.Should().BeTrue();
        validator.Validate(new ContentSearchRequest { Query = "report", Cursor = "bad!" }).IsValid.Should().BeFalse();
    }
