...
| Variable       | Value                                     |
| -------------- | ----------------------------------------- |
| `UDS_SSL_CERT` | Absolute path to an mkcert **cert** PEM   |
| `UDS_SSL_KEY`  | Absolute path to the matching **key** PEM |
...
| ------------------------------------------- | ------------------------------------------- |
| `https://dev-uds.blocksdevelopers.com:4000` | `https://dev-uds.blocksdevelopers.com:5000` |
...
2. Generate a cert + key for the dev domain (keep them OUTSIDE the repo, e.g. `~/.certs`):
   ...
   ```bash
   mkcert dev-uds.blocksdevelopers.com
   ```

3. Add a hosts-file entry so the domain resolves locally:

   ```
   127.0.0.1 dev-uds.blocksdevelopers.com
   ```
...
4. Set the two environment variables (point at the files from step 2):
   ...
   **Windows (PowerShell, persists for the user — reopen the terminal after):**
   ...
   ```powershell
   setx UDS_SSL_CERT "C:\Users\<you>\.certs\dev-uds.blocksdevelopers.com.pem"
   setx UDS_SSL_KEY  "C:\Users\<you>\.certs\dev-uds.blocksdevelopers.com-key.pem"
   ```

   **macOS / Linux (add to `~/.zshrc` or `~/.bashrc`):**

   ```bash
   export UDS_SSL_CERT="$HOME/.certs/dev-uds.blocksdevelopers.com.pem"
   export UDS_SSL_KEY="$HOME/.certs/dev-uds.blocksdevelopers.com-key.pem"
   ```

### Behavior matrix

| `UDS_SSL_CERT` / `UDS_SSL_KEY` | Files exist | Result                                            |
| ------------------------------ | ----------- | ------------------------------------------------- |
