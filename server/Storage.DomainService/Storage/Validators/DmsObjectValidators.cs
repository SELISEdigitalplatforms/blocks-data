...
namespace Storage.DomainService.Storage.Validators
{
    ...
    internal static class DmsValidationRules
    {
        ...
        internal static bool BeADecodableCursor(string? cursor) =>
            string.IsNullOrEmpty(cursor) || ObjectCursor.Decode(cursor) is not null;
    ...
    public class GetObjectRequestValidator : AbstractValidator<GetObjectRequest>
    {
        public GetObjectRequestValidator()
        {
    ...
    public class ObjectSearchRequestValidator : AbstractValidator<ObjectSearchRequest>
    {
        public ObjectSearchRequestValidator()
        {
    ...
    public class SharedObjectRequestValidator : AbstractValidator<SharedObjectRequest>
    {
        public SharedObjectRequestValidator()
        {
            RuleFor(r => r.Limit).InclusiveBetween(1, DmsValidationRules.MaxPageSize);
            RuleFor(r => r.Cursor).Must(DmsValidationRules.BeADecodableCursor).WithMessage("Cursor is not a valid continuation token.");
        }
