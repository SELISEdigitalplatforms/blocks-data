...
describe("GraphLogService", () => {
  ...
  it("passes outcome, status-code, and sorting controls to request history", async () => {
    ...
    expect(http.get).toHaveBeenCalledWith(
      `${GRAPH_LOG_ENDPOINTS.HISTORY}?PageNo=2&PageSize=25&From=2026-09-01&To=2026-09-08&OperationType=query&Outcome=denied&StatusCode=401&FailureKind=authentication&SortBy=schema&SortDescending=false`,
    );
  });
});
