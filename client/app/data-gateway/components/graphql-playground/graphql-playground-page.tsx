...
import { Play, Trash2, Keyboard, BookOpen } from "lucide-react";
...
export const GraphQLPlaygroundPage = () => {
  const [query, setQuery] = useState(`# Write your GraphQL query/mutation here`);
  ...
  const isSchemasDrawerLoading =
    ...
    (isSchemasIntrospectionPending ||
      (isSchemasIntrospectionFetching && schemasIntrospectionData === undefined));
  ...
  const parseOperations = useCallback((text: string) => {
    ...
    let currentOperation: { type: string; name: string; startLine: number; text: string } | null =
      null;
    let braceDepth = 0;
  ...
  const injectMockDataTag = useCallback((queryText: string): string => {
    ...
    // Step 1: Find the input: { block using brace-depth tracking
    const inputStartMatch = queryText.match(/(input\s*:\s*\{)/i);
    if (!inputStartMatch || inputStartMatch.index === undefined) {
      return queryText;
    }

    const openBraceIndex = inputStartMatch.index + inputStartMatch[0].length - 1;
    let depth = 1;
    let pos = openBraceIndex + 1;

    while (pos < queryText.length && depth > 0) {
      if (queryText[pos] === "{") depth++;
      else if (queryText[pos] === "}") depth--;
      if (depth > 0) pos++;
    }

    if (depth !== 0) return queryText; // Malformed query

    // pos is now the index of the matching closing } of input: {}
    const inputContent = queryText.substring(openBraceIndex + 1, pos);

    // Step 2: Find Tags only at the TOP LEVEL of inputContent (depth 0).
    // Nested DTOs (e.g. Assignee: [{Tags: [...]}]) are at depth > 0 and must be skipped.
    let scanDepth = 0;
    let topLevelTagsStart = -1;
    let topLevelTagsEnd = -1;
    let k = 0;

    while (k < inputContent.length) {
      const ch = inputContent[k];
      ...
      // Only inspect for Tags at the top level of the input block
      if (scanDepth === 0) {
        const rest = inputContent.substring(k);
        const tagsKeyMatch = rest.match(/^Tags\s*:\s*\[/i);

        if (tagsKeyMatch) {
          topLevelTagsStart = k;

          // Find the closing ] for this Tags array using bracket-depth tracking
          const bracketOpenOffset = tagsKeyMatch[0].lastIndexOf("[");
          let arrDepth = 1;
          let j = k + bracketOpenOffset + 1;

          while (j < inputContent.length && arrDepth > 0) {
            if (inputContent[j] === "[") arrDepth++;
            else if (inputContent[j] === "]") arrDepth--;
            if (arrDepth > 0) j++;
          }

          topLevelTagsEnd = j + 1; // exclusive end (past the closing ])
          break;
        }
      }
    ...
    if (topLevelTagsStart !== -1) {
      // Case A: Tags exists at the top level — add "mock-data" if missing
      const tagsStr = inputContent.substring(topLevelTagsStart, topLevelTagsEnd);
      ...
      if (hasMockData) return queryText;
      ...
      const newInputContent =
        inputContent.substring(0, topLevelTagsStart) +
        newTagsStr +
        inputContent.substring(topLevelTagsEnd);

      return (
        queryText.substring(0, openBraceIndex + 1) + newInputContent + queryText.substring(pos)
      );
    } else {
      // Case B: Tags doesn't exist at top level — inject it before the closing brace
      const newContent = inputContent.trim()
        ? `${inputContent.trimEnd()}\n    Tags: ["mock-data"]\n  `
        : `\n    Tags: ["mock-data"]\n  `;

      return queryText.substring(0, openBraceIndex + 1) + newContent + queryText.substring(pos);
    }
  }, []);
  ...
  const executeOperation = useCallback(
    async (operationText: string) => {
      try {
        ...
        const data = await executeGraphQL({ projectShortKey, query: modifiedQuery });
        ...
        <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
          ...
          <div className="flex w-full flex-col md:w-1/2">
            {responses.length === 0 ? (
              ...
              <>
                <div className="flex h-12 items-center border-b bg-muted px-4">
                  <span className="text-sm font-medium">{responses[0].name}</span>
                </div>
                <div className="flex-1 overflow-hidden">
                  <GraphqlMonacoEditor
                    height="100%"
