...
async function persistSuiteSession(page: Page) {
  ...
  await page.context().storageState({ path: DATA_SESSION_PATH })
}
...
export async function openSharedProjectDashboard(page: Page) {
  ...
  if (!fixture?.itemId) {
    throw new Error(
      "Missing fixtures/data-project.json (or itemId) — run the data-setup project first " +
        "(suite.setup.spec.ts).",
    )
  }
  ...
  try {
    await waitForDataDashboardReady(page, fixture.projectName)
    await persistSuiteSession(page)
  } catch (error) {
