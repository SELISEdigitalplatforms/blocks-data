...
namespace Storage.DomainService.Services
{
    ...
    /// </remarks>
    public class ObjectListingService : IObjectListingService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var position = ObjectCursor.Decode(cursor);
            ...
            {
                var last = visible[^1];
                page.NextCursor = new ObjectCursor { Type = last.Type, Name = last.Name, ItemId = last.ItemId }.Encode();
            }
        ...
        private async Task<bool> CanViewAsync(FileDirectory parent, CancellationToken cancellationToken)
            => await _resolver.ResolveAsync(Describe(parent), ObjectPermission.View, cancellationToken);
        ...
        private async Task<List<ChildRow>> ReadMergedSliceAsync(
            string parentId, ObjectCursor? position, int take, StructureType? type, string? search, CancellationToken cancellationToken)
        {
        ...
        private sealed class ChildRow
        {
            ...
            public required ObjectResourceDescriptor Descriptor { get; init; }
            ...
            public static ChildRow From(FileDirectory d) => new()
            {
                ...
                Descriptor = new ObjectResourceDescriptor
                {
                    ResourceId = d.ItemId,
        ...
        private sealed class ChildRowComparer : IComparer<ChildRow>
        {
            ...
            public int Compare(ChildRow? x, ChildRow? y)
            {
                ...
                return ObjectCursor.Compare(x.Type, x.Name, x.ItemId, y.Type, y.Name, y.ItemId);
            }
        }
    }
}
